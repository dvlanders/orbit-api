...
const endpointCall = async (
  ...
  return await supertest(app)
    .get(`/transfer/fiat-to-crypto?apiKeyId=${apiKey}&id=${recordId}`)
    .set({
...
describe("GET /transfer/fiat-to-crypto", function () {
  describe("Valid Get Transfer Test", function () {
    it("should have status code 200 for valid record Id", async () => {
      const txRes = await endpointCall(getFiatToCryptoTransferParams.RECORD_ID);
      ...
      expect(tx.transferDetails.id).toBe(
        getFiatToCryptoTransferParams.RECORD_ID
      );
    }, 10000);
  });
