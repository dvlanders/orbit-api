...
const processOfframpTransferEvent = async (event) => {
  const {
    id,
    state,
    amount,
  ...
  try{
    ...
    if (!offrampRecord) return await createLog("webhook/processOfframpTransferEvent", null, `Offramp record not found for client_reference_id ${client_reference_id}. The record is most likely in dev_production database.`);
    ...
    await createLog(
      "webhooks/bridge/processOfframpTransferEvent",
      null,
