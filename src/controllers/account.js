...
// exports.createFiatCheckbookAccount = async (req, res) => {
// 	if (req.method !== 'POST') {
// 		return res.status(405).json({ error: 'Method not allowed' });
// 	}
...
// 	// fields required for chekckbook creation: account number, routing number, type
// 	let { accountNumber, routingNumber, type } = fields
// 	// fields validation
// 	const requiredFields = ["accountNumber", "routingNumber", "type"]
// 	const acceptedFields = {
// 		accountNumber: "string",
// 		routingNumber: "string",
// 		type: (value) => inStringEnum(value, ["CHECKING", "SAVINGS"]),
// 	}
// 	const { missingFields, invalidFields } = fieldsValidation(fields, requiredFields, acceptedFields)
// 	if (missingFields.length > 0) {
// 		return res.status(400).json({ error: 'Missing required fields', missingFields });
...
// 		await createSingleCheckbookUser(user, userId, checkbookUserId, "DESTINATION") // FIXME: Should we grab the specific error enums from this and return a more nuanced response?
// 		// create checkbook account
// 		await insertPreCheckbookAccountRecord(userId, virtualAccount) // FIXME: Should we grab the specific error enums from this and return a more nuanced response?
