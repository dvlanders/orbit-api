...
import { bridgeFieldsToDatabaseFields } from "../utils";

const BRIDGE_API_KEY = process.env.BRIDGE_API_KEY;
const BRIDGE_URL = process.env.BRIDGE_URL;

enum createBridgeCustomerErrorType {
    RECORD_NOT_FOUND,
    INVALID_FIELD,
    INTERNAL_ERROR,
}
...
class createBridgeCustomerError extends Error {
    type: createBridgeCustomerErrorType;
    rawResponse: any

    constructor(type: createBridgeCustomerErrorType, message: string, rawResponse?: any) {
        super(message);
        this.type = type;
...
function getEndorsementStatus(endorsements: Endorsement[], name: string): string | undefined {
...
/*
    endpoint for passing bridge kyc
*/
export async function createIndividualBridgeCustomer(profileId: string, userId: string) {
